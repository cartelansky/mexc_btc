...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
        ...
        for symbol in symbols {
                ...
                (symbol["symbol"].as_str(), symbol["quoteAsset"].as_str())
            {
                if quote_asset == "BTC" {
                    ...
                    coins.push(format!("MEXC:{}BTC", base_asset));
                }
            }
        }
    }
    ...
    if coins.is_empty() {
        println!("BTC çiftleri bulunamadı. API yanıtını kontrol edin.");
        return Ok(());
    }

    coins.sort_by(|a, b| {
        let a_name = a.split(':').nth(1).unwrap_or("").trim_end_matches("BTC");
        let b_name = b.split(':').nth(1).unwrap_or("").trim_end_matches("BTC");
    ...
    let mut file = File::create("mexc_btc_markets.txt")?;
    for coin in &coins {
        writeln!(file, "{}", coin)?;
    }

    println!("BTC spot piyasası coin listesi başarıyla oluşturuldu ve kaydedildi.");
    println!("Toplam {} adet BTC çifti bulundu.", coins.len());
    Ok(())
}
